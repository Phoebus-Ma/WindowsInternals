...
void CChildView::
AddThread(unique_ptr<CThread> &thread)
{
    CString number;
...
void CChildView::
UpdateThread(int n, const CThread *thread)
{
    if (thread == nullptr)
...
PCWSTR CChildView::
ActivityLevelToString(ActivityLevel level)
{
...
PCWSTR CChildView::
ThreadPriorityToString(int priority)
{
    static const PCWSTR Levels[] = {
        L"Idle",
        L"Lowest",
...
BOOL CChildView::
QueueItemForThreadPool(int busyPercent)
{
...
vector<pair<CThread *, int>> CChildView::
GetSelectedThreads() const
{
...
void CChildView::
OnUpdateThreadActivate(CCmdUI *pCmdUI)
{
...
void CChildView::
OnUpdateThreadDeactivate(CCmdUI *pCmdUI)
{
    pCmdUI->Enable(m_List.GetSelectedCount() > 0);
}

void CChildView::
OnChangeThreadActivity(UINT id)
{
...
void CChildView::
OnUpdateChangeThreadActivity(CCmdUI *pCmdUI)
{
    pCmdUI->Enable(m_List.GetSelectedCount() > 0);
}

void CChildView::
OnProcessCreatethread()
{
...
void CChildView::
OnUpdateProcessCreatethread(CCmdUI *pCmdUI)
{
    pCmdUI->Enable(m_Threads.size() < 64);
}

void CChildView::
OnProcessCreate4threads()
{
    for (int i = 0; i < 4; i++)
    {
        AddThread(CreateThread());
...
void CChildView::
OnUpdateProcessCreate4threads(CCmdUI *pCmdUI)
{
    pCmdUI->Enable(m_Threads.size() < 60);
}

void CChildView::
OnChangeThreadPriority(UINT id)
{
    for (auto &item : GetSelectedThreads())
    {
...
void CChildView::
OnUpdateChangeThreadPriority(CCmdUI *pCmdUI)
{
    pCmdUI->Enable(m_List.GetFirstSelectedItemPosition() != nullptr);
}

int CChildView::
IndexToPriority(int index)
{
...
void CChildView::
OnOptionsAutorefreshthreadindices()
{
...
void CChildView::
OnUpdateOptionsAutorefreshthreadindices(CCmdUI *pCmdUI)
{
    pCmdUI->SetCheck(m_AutoRefreshThreadIndices);
}

void CChildView::
OnCpusetsSystemcpuset()
{
...
void CChildView::
OnUpdateCpusetsThreadselectedcpuset(CCmdUI *pCmdUI)
{
    pCmdUI->Enable(m_List.GetSelectedCount() == 1);
}

void CChildView::
OnProcessQueuethreadpoolwork()
{
...
void CChildView::
OnUpdateStatusThreads(CCmdUI *pCmdUI)
{
    CString text;
...
UpdateCPUTimes()
{
    ...
    {
        cpu = static_cast<ULONG>(
            (total - m_LastProcessTimes) * 1000000LL        /
            ((counter.QuadPart - m_LastQueryCount.QuadPart) *
            1000000LL / m_QueryFrequency.QuadPart)
        );
...
void CChildView::
OnUpdateStatusProcessCpu(CCmdUI *pCmdUI)
{
    CString text;
...
void CChildView::
OnUpdateCpuUpdateInterval(CCmdUI *pCmdUI)
{
